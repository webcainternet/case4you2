...
<div class="box-container">
  ...
  <?php if ($attention) { ?>
  <div class="attention"><?php echo $attention; ?><img src="catalog/view/theme/theme254/image/remove-small.png" alt="" class="close" /></div>
  <?php } ?>
  <?php if ($success) { ?>
  <div class="success"><?php echo $success; ?><img src="catalog/view/theme/theme254/image/remove-small.png" alt="" class="close" /></div>
  <?php } ?>
  <?php if ($error_warning) { ?>
  <div class="warning"><?php echo $error_warning; ?><img src="catalog/view/theme/theme254/image/remove-small.png" alt="" class="close" /></div>
  <?php } ?>
    ...
    <div class="cart-info">
      ...
      <table>
        ...
        <tbody>
          ...
          <tr>
            <td class="image"><img src="<?php echo $product['thumb']; ?>" data-fullimage="<?php echo $product['image']; ?>"></td>
          ...
          <tr>
            ...
            <td class="quantity"><input type="text" name="" value="1" size="1" disabled="disabled" />
              &nbsp;<a href="<?php echo $vouchers['remove']; ?>"><img src="catalog/view/theme/theme254/image/remove.png" alt="<?php echo $text_remove; ?>" title="<?php echo $button_remove; ?>" /></a></td>
...
$('#button-quote').live('click', function() {
	$.ajax({
		...
		beforeSend: function() {
			...
			$('#button-quote').after('<span class="wait">&nbsp;<img src="catalog/view/theme/theme254/image/loading.gif" alt="" /></span>');
		},
		complete: function() {
		...
		success: function(json) {
			...
			if (json['error']) {
				if (json['error']['warning']) {
					$('#notification').html('<div class="warning" style="display: none;">' + json['error']['warning'] + '<img src="catalog/view/theme/theme254/image/close-1.png" alt="" class="close" /></div>');
...
$('select[name=\'country_id\']').bind('change', function() {
	$.ajax({
		...
		dataType: 'json',
		beforeSend: function() {
			$('select[name=\'country_id\']').after('<span class="wait">&nbsp;<img src="catalog/view/theme/theme254/image/loading.gif" alt="" /></span>');
		},
		complete: function() {
			$('.wait').remove();
		},
...
function moveImage(e){
    $('.js-moving-image').css({left:e.pageX, top:e.pageY});
};

var leaveTimeout = null;

$('body').on('mouseenter', '[data-fullimage]', function(e){
    if (leaveTimeout) {
        ...
        $div.addClass('js-moving-image');

        var $image = $(document.createElement('img'));
        $image.attr('src', $(this).attr('data-fullimage'));
        $div.append($image);

        $('body').append($div);

        $(document).on('mousemove', 'body', moveImage);
    }
});

$('body').on('mouseleave', '[data-fullimage]', function(e){
    leaveTimeout = setTimeout(function(){
        $('.js-moving-image').remove();
        $(document).off('mousemove', 'body', moveImage);
        clearTimeout(leaveTimeout);
        leaveTimeout = null;
    }, 200);
});
