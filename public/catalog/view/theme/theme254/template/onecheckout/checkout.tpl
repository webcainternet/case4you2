...
<div id="content"><?php echo $content_top; ?>
  ...
  <?php if (!$logged) { ?>
  <div id="checkout">
  	<a id="login-show" style="color:#6aa11a;"><?php echo $text_checkout_option; ?></a>
  	<div id="login"></div>
  </div>
  <?php } ?>
		...
		success: function(json) {
			...
			if (json['error']) {
				...
				if (json['error']['company']) {
                                        $('#payment-address input[name=\'company\'] + br').after('<span id="error_company" class="error">' + json['error']['company'] + '</span>');
                                }
...
$('#button-confirmorder').live('click', function() {
    $('.valiwarning').remove();
    if ($('input[name=company]').val() == '' && $('#payment-existing').length < 1) {
        $('<br><small class="valiwarning">O campo CPF/CNPJ é obrigatório</small>').insertAfter('input[name=company]');
        return false;
    }
