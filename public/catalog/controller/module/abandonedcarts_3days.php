...
class ControllerModuleAbandonedcarts3Days extends Controller
{
    public function sendReminder()  {
		$log = new Log("abandonedcarts3days_log.txt");
		...
		$setting = $this->model_module_abandonedcarts_3days->getSetting('AbandonedCarts3Days', $store_id);
        if (!empty($setting['AbandonedCarts3Days']['Enabled']) && $setting['AbandonedCarts3Days']['Enabled'] == 'yes' && !empty($setting['AbandonedCarts3Days']['ScheduleEnabled']) && $setting['AbandonedCarts3Days']['ScheduleEnabled'] == 'yes') {
			$dayLimit = $setting['AbandonedCarts3Days']['Delay'];
			$results = $this->model_module_abandonedcarts_3days->getCarts($dayLimit);
			$usedEmails = array();
				...
				if (isset($result['customer_info']['email']) && isset($result['customer_info']['firstname']) && isset($result['customer_info']['lastname'])) {
					...
					if (!in_array($result['customer_info']['email'], $usedEmails,true)) {
						...
						if (isset($result['customer_info']['language'])) {
							$Subject = $setting['AbandonedCarts3Days']['Subject'][$result['customer_info']['language']];
							$Message = html_entity_decode($setting['AbandonedCarts3Days']['Message'][$result['customer_info']['language']]);
						} else {
							$languages = $this->getLanguages();
							$firstLanguage = array_shift($languages);
							$firstLanguageCode = $firstLanguage['code'];
							$Subject = $setting['AbandonedCarts3Days']['Subject'][$firstLanguageCode];
							$Message = html_entity_decode($setting['AbandonedCarts3Days']['Message'][$firstLanguageCode]);
						}
						...
						$width					  = (isset($setting['AbandonedCarts3Days']['ProductWidth'])) ? $setting['AbandonedCarts3Days']['ProductWidth'] : '60';
						$height					 = (isset($setting['AbandonedCarts3Days']['ProductWidth'])) ? $setting['AbandonedCarts3Days']['ProductHeight'] : '60';
						...
						if ($setting['AbandonedCarts3Days']['DiscountType']=='N') {
							// do nothing here
						} else {
							if ($setting['AbandonedCarts3Days']['DiscountApply']=='all_products') {
								$DiscountCode			   = $this->generateuniquerandomcouponcode();
								$TimeEnd					=  time() + $setting['AbandonedCarts3Days']['DiscountValidity'] * 24 * 60 * 60;
								$CouponData				 = array('name' => 'AbCart [' . $result['customer_info']['email'].']',
								'code'			=> $DiscountCode,
								'discount'		=> $setting['AbandonedCarts3Days']['Discount'],
								'type'			=> $setting['AbandonedCarts3Days']['DiscountType'],
								'total'		   => $setting['AbandonedCarts3Days']['TotalAmount'],
								'logged'		  => '0',
								'shipping'		=> '0',
								...
								'uses_customer'   => '1',
								'status'		  => '1');
								$this->model_module_abandonedcarts_3days->addCoupon($CouponData);
						...
						if (!($setting['AbandonedCarts3Days']['DiscountType']=='N')) {
							...
							$replacements[4] = $setting['AbandonedCarts3Days']['Discount'];
							$replacements[5] = $setting['AbandonedCarts3Days']['TotalAmount'];
						...
						if (isset($setting['AbandonedCarts3Days']['RemoveAfterSend']))
							$run_query = $this->db->query("DELETE FROM `" . DB_PREFIX . "abandonedcarts_3days` WHERE `id`=".(int)$result['id']);
					}
				}
			}
        } else {
