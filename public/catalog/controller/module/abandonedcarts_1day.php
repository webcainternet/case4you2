...
class ControllerModuleAbandonedcarts1Day extends Controller
{
    public function sendReminder()  {
		$log = new Log("abandonedcarts1day_log.txt");
		...
		$setting = $this->model_module_abandonedcarts_1day->getSetting('AbandonedCarts1Day', $store_id);
        if (!empty($setting['AbandonedCarts1Day']['Enabled']) && $setting['AbandonedCarts1Day']['Enabled'] == 'yes' && !empty($setting['AbandonedCarts1Day']['ScheduleEnabled']) && $setting['AbandonedCarts1Day']['ScheduleEnabled'] == 'yes') {
			$dayLimit = $setting['AbandonedCarts1Day']['Delay'];
			$results = $this->model_module_abandonedcarts_1day->getCarts($dayLimit);
			$usedEmails = array();
				...
				if (isset($result['customer_info']['email']) && isset($result['customer_info']['firstname']) && isset($result['customer_info']['lastname'])) {
					...
					if (!in_array($result['customer_info']['email'], $usedEmails,true)) {
						...
						if (isset($result['customer_info']['language'])) {
							$Subject = $setting['AbandonedCarts1Day']['Subject'][$result['customer_info']['language']];
							$Message = html_entity_decode($setting['AbandonedCarts1Day']['Message'][$result['customer_info']['language']]);
						} else {
							$languages = $this->getLanguages();
							$firstLanguage = array_shift($languages);
							$firstLanguageCode = $firstLanguage['code'];
							$Subject = $setting['AbandonedCarts1Day']['Subject'][$firstLanguageCode];
							$Message = html_entity_decode($setting['AbandonedCarts1Day']['Message'][$firstLanguageCode]);
						}
						...
						$width					  = (isset($setting['AbandonedCarts1Day']['ProductWidth'])) ? $setting['AbandonedCarts1Day']['ProductWidth'] : '60';
						$height					 = (isset($setting['AbandonedCarts1Day']['ProductWidth'])) ? $setting['AbandonedCarts1Day']['ProductHeight'] : '60';
						...
						if ($setting['AbandonedCarts1Day']['DiscountType']=='N') {
							// do nothing here
						} else {
							if ($setting['AbandonedCarts1Day']['DiscountApply']=='all_products') {
								$DiscountCode			   = $this->generateuniquerandomcouponcode();
								$TimeEnd					=  time() + $setting['AbandonedCarts1Day']['DiscountValidity'] * 24 * 60 * 60;
								$CouponData				 = array('name' => 'AbCart [' . $result['customer_info']['email'].']',
								'code'			=> $DiscountCode,
								'discount'		=> $setting['AbandonedCarts1Day']['Discount'],
								'type'			=> $setting['AbandonedCarts1Day']['DiscountType'],
								'total'		   => $setting['AbandonedCarts1Day']['TotalAmount'],
								'logged'		  => '0',
								'shipping'		=> '0',
								...
								'uses_customer'   => '1',
								'status'		  => '1');
								$this->model_module_abandonedcarts_1day->addCoupon($CouponData);
						...
						if (!($setting['AbandonedCarts1Day']['DiscountType']=='N')) {
							...
							$replacements[4] = $setting['AbandonedCarts1Day']['Discount'];
							$replacements[5] = $setting['AbandonedCarts1Day']['TotalAmount'];
						...
						if (isset($setting['AbandonedCarts1Day']['RemoveAfterSend']))
							$run_query = $this->db->query("DELETE FROM `" . DB_PREFIX . "abandonedcarts_1day` WHERE `id`=".(int)$result['id']);
					}
				}
			}
        } else {
