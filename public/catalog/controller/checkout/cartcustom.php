    	...
    	if ($this->cart->hasProducts() || !empty($this->session->data['vouchers'])) {
			...
			$products = $this->cart->getProducts();
      		...
      		foreach ($products as $product) {
				...
				if ($product['image']) {
					$cart_image_width = (int) $this->config->get('config_image_cart_width');
					$cart_image_height = (int) $this->config->get('config_image_cart_height');
					$image = $this->model_tool_image->resize($product['image'], $cart_image_width, $cart_image_height);
					$full_image = $this->model_tool_image->resize($product['image'], $cart_image_width * 5, $cart_image_height * 5);
				} else {
					$image = '';
				}
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/checkout/cartcustom.tpl')) {
				$this->template = $this->config->get('config_template') . '/template/checkout/cartcustom.tpl';
			} else {
				$this->template = 'default/template/checkout/cartcustom.tpl';
			}
