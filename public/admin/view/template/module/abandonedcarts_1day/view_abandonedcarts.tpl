              ...
              <tbody>
				<tr>
                  ...
                  <td>
					  ...
					  <a data-event-id="<?php echo $ab['id']; ?>" href="<?php echo $this->url->link('module/abandonedcarts_1day/sendreminder', 'token=' . $this->session->data['token']."&id=".$ab['id']."&store_id=".$this->request->get['store_id'], 'SSL'); ?>" class="btn btn-small"><i class="icon-envelope"></i> Send reminder</a>
					  <?php } else { ?>
...
<script>
   ...
   function removeAll() {
		...
		if (r==true) {
			$.ajax({
				url: 'index.php?route=module/abandonedcarts_1day/removeallrecords&store=<?php echo $this->request->get['store_id']; ?>&token=<?php echo $this->session->data['token']; ?>',
				type: 'post',
   ...
   function removeAllEmpty() {
		...
		if (r==true) {
			$.ajax({
				url: 'index.php?route=module/abandonedcarts_1day/removeallemptyrecords&store=<?php echo $this->request->get['store_id']; ?>&token=<?php echo $this->session->data['token']; ?>',
				type: 'post',
				data: {'remove': r, 'store' : <?php echo $this->request->get['store_id']; ?> },
				success: function(response) {
				location.reload();
