        	...
        	<ul class="dropdown-menu">
            	<?php foreach ($stores as $st) { ?>
            		<li <?php if($store['store_id'] == $st['store_id']){ echo 'class="disabled"';}?>>
            			<a href="index.php?route=module/abandonedcarts_3days&store_id=<?php echo $st['store_id'];?>&token=<?php echo $this->session->data['token']; ?>">
            				<?php echo $st['name']; ?>
            			</a>
            		</li>
            	<?php } ?>
        ...
        <div class="tabbable">
         ...
         <div class="tab-content">
        	<div id="abandonedCarts" class="tab-pane active">
              <?php require_once(DIR_APPLICATION.'view/template/module/abandonedcarts_3days/tab_abandonedcarts.php'); ?>
            </div>
            <div id="mail" class="tab-pane">
              <?php require_once(DIR_APPLICATION.'view/template/module/abandonedcarts_3days/tab_mail.php'); ?>
            </div>
			<div id="controlpanel" class="tab-pane">
              <?php require_once(DIR_APPLICATION.'view/template/module/abandonedcarts_3days/tab_panel.php'); ?>
            </div>
            <div id="givenCoupons" class="tab-pane"></div>
            <div id="usedCoupons" class="tab-pane"></div>
			<div id="supp" class="tab-pane">
              <?php require_once(DIR_APPLICATION.'view/template/module/abandonedcarts_3days/tab_support.php'); ?>
            </div>
          </div><!-- /.tab-content -->
        </div><!-- /.tabbable -->
      </form>
...
$(document).ready(refreshData());
  function refreshData(){
      $.ajax({
         url: "index.php?route=module/abandonedcarts_3days/givenCoupons&token=<?php echo $this->session->data['token']; ?>",
         type: 'get',
         dataType: 'html',
      ...
      $.ajax({
         url: "index.php?route=module/abandonedcarts_3days/usedCoupons&token=<?php echo $this->session->data['token']; ?>",
         type: 'get',
         dataType: 'html',
         success: function(data) {
...
function upgrade() {
	$.ajax({
		url: "index.php?route=module/abandonedcarts_3days/upgrade&token=<?php echo $this->session->data['token']; ?>",
		type: 'get',
		dataType: 'json',
