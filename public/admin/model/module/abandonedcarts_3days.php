<?php
class ModelModuleAbandonedcarts3Days extends Model {
	/* upgrade start (from older versions than 1.4)	*/
	public function upgrade() {
		$query =  $this->db->query("SELECT * FROM `" . DB_PREFIX . "abandonedcarts_3days` ORDER BY `id` DESC");
	...
	public function updateCart($id, $customer, $cart) {
		$this->db->query("UPDATE `" . DB_PREFIX . "abandonedcarts_3days` SET `cart`='".$this->db->escape($cart)."', `customer_info`='".$this->db->escape($customer)."'
		WHERE `id`=".$id);
	}
	...
	public function install() {
		$this->db->query("CREATE TABLE IF NOT EXISTS `" . DB_PREFIX . "abandonedcarts_3days` (
	...
	public function uninstall() {
		$this->db->query("DROP TABLE IF EXISTS `" . DB_PREFIX . "abandonedcarts_3days`");
	}
	...
	public function viewAbandonedCarts($page=1, $limit=8, $store=0,$sort="id", $order="DESC") {
		...
		if ($page) {
			...
			$query =  $this->db->query("SELECT * FROM `" . DB_PREFIX . "abandonedcarts_3days`
	...
	public function getTotalAbandonedCarts($store=0){
		$query = $this->db->query("SELECT COUNT(*) as `count`  FROM `" . DB_PREFIX . "abandonedcarts_3days` WHERE `store_id`=".$store);
