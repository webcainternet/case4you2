...
class ModelModuleAbandonedcarts extends Model {
	...
	public function upgrade() {
		$query =  $this->db->query("SELECT * FROM `" . DB_PREFIX . "abandonedcarts` ORDER BY `id` DESC");
	...
	public function updateCart($id, $customer, $cart) {
		$this->db->query("UPDATE `" . DB_PREFIX . "abandonedcarts` SET `cart`='".$this->db->escape($cart)."', `customer_info`='".$this->db->escape($customer)."'
		WHERE `id`=".$id);
	}
