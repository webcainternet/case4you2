...
class ControllerModuleAbandonedcarts extends Controller {
	...
	public function index() {
		$this->language->load('module/abandonedcarts');
		...
		$check_update = $this->db->query("SHOW COLUMNS FROM `" . DB_PREFIX . "abandonedcarts` LIKE 'store_id'");
		if (!$check_update->rows)
			$this->db->query("ALTER TABLE `" . DB_PREFIX . "abandonedcarts`
  						ADD `store_id` TINYINT NOT NULL DEFAULT 0");
		// check for the multi-store variable

		if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
			if (!$this->user->hasPermission('modify', 'module/abandonedcarts')) {
				$this->validate();
			...
			$this->model_module_abandonedcarts->editSetting('AbandonedCarts', $this->request->post, $this->request->post['store_id']);
			...
			$this->redirect($this->url->link('module/abandonedcarts', 'token=' . $this->session->data['token'] . '&tab='.$selectedTab .'&store_id='.$store['store_id'], 'SSL'));
		}
		...
		$this->data['stores'] = array_merge(array(0 => array('store_id' => '0', 'name' => $this->config->get('config_name') . ' (' . $this->data['text_default'].')', 'url' => HTTP_SERVER, 'ssl' => HTTPS_SERVER)), $this->model_setting_store->getStores());
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('module/abandonedcarts', 'token=' . $this->session->data['token'], 'SSL'),
      		'separator' => ' :: '
   		);
		...
		$this->data['action']			  = $this->url->link('module/abandonedcarts', 'token=' . $this->session->data['token'], 'SSL');
		$this->data['cancel']			  = $this->url->link('extension/module', 'token=' . $this->session->data['token'], 'SSL');
		$this->data['data']				= $this->model_module_abandonedcarts->getSetting('AbandonedCarts',$store['store_id']);
		$this->data['currenttemplate']	 = $this->config->get('config_template');

		$this->data['layouts']			 = $this->model_design_layout->getLayouts();
		$this->template					= 'module/abandonedcarts.tpl';
	...
	public function install() {
		$this->load->model('module/abandonedcarts');
		$this->load->model('setting/store');
		$this->model_module_abandonedcarts->install();
		/* new data */
		$stores = $this->model_setting_store->getStores();
			foreach ($stores as $store) {
				$data = $this->model_module_abandonedcarts->getSetting('AbandonedCarts', $store['store_id']);
				$data['AbandonedCarts']['Updated'] = 'yes';
				$this->model_module_abandonedcarts->editSetting('AbandonedCarts', $data, $store['store_id']);
			}
			$data = $this->model_module_abandonedcarts->getSetting('AbandonedCarts', '0');
			$data['AbandonedCarts']['Updated'] = 'yes';
			$this->model_module_abandonedcarts->editSetting('AbandonedCarts', $data, '0');
		/* new data */
	}
	...
	public function upgrade() {
		...
		$this->load->model('setting/store');
		$this->load->model('module/abandonedcarts');
		$upgrade = $this->model_module_abandonedcarts->upgrade();
		if ($upgrade) {
			$stores = $this->model_setting_store->getStores();
			foreach ($stores as $store) {
				$data = $this->model_module_abandonedcarts->getSetting('AbandonedCarts', $store['store_id']);
	...
	public function uninstall() {
		...
		$this->load->model('setting/store');
		$this->model_setting_setting->deleteSetting('AbandonedCarts',0);
		$stores = $this->model_setting_store->getStores();
		foreach ($stores as $store) {
			$this->model_setting_setting->deleteSetting('AbandonedCarts', $store['store_id']);
		}

        $this->load->model('module/abandonedcarts');
        $this->model_module_abandonedcarts->uninstall();
	}
	...
	public function getabandonedcarts() {
        ...
        $pagination->total        = $this->model_module_abandonedcarts->getTotalAbandonedCarts($this->request->get['store_id']);
        $pagination->page         = $page;
        $pagination->limit        = 8;
        $pagination->text         = $this->language->get('text_pagination');
        $pagination->url          = $this->url->link('module/abandonedcarts/getabandonedcarts','token=' . $this->session->data['token'].'&page={page}&store_id='.$this->request->get['store_id'], 'SSL');
        $this->data['sources']    = $this->model_module_abandonedcarts->viewAbandonedCarts($page, $pagination->limit, $this->request->get['store_id']);
        $this->data['limit']      = $pagination->limit;
        $this->template           = 'module/abandonedcarts/view_abandonedcarts.tpl';
	...
	public function removeabandonedcart() {
		if (isset($_POST['cart_id'])) {
				$run_query = $this->db->query("DELETE FROM `" . DB_PREFIX . "abandonedcarts` WHERE `id`=".(int)$_POST['cart_id']);
	...
	public function sendreminder() {
		...
		$firstLanguage					 = array_shift($languages);
		$this->data['firstLanguageCode']   = $firstLanguage['code'];
		$this->data['data']				= $this->model_module_abandonedcarts->getSetting('AbandonedCarts', $this->request->get['store_id']);
		$this->data['id']				  = $_GET['id'];
		$this->data['currency']			= $this->config->get('config_currency');
		$this->data['result']			  = $this->model_module_abandonedcarts->getCartInfo($this->data['id']);
		$this->data['currenttemplate']	 = $this->config->get('config_template');
		$this->data['layouts']			 = $this->model_design_layout->getLayouts();
        $this->template					= 'module/abandonedcarts/send_reminder.tpl';
	...
	public function sendcustomemail() {
		if (isset($_POST) && isset($_POST['ABcart_id'])) {
			...
			$result					 = $this->model_module_abandonedcarts->getCartInfo($_POST['ABcart_id']);
			$this->request->get['store_id'] = $result['store_id'];
			$setting					= $this->model_module_abandonedcarts->getSetting('AbandonedCarts', $this->request->get['store_id']);
			...
			$emailResult = $this->model_module_abandonedcarts->sendMail($MailData);
 			if ($emailResult) {
				...
				if (isset($_POST['RemoveAfterSend']))
					$run_query = $this->db->query("DELETE FROM `" . DB_PREFIX . "abandonedcarts` WHERE `id`=".(int)$_POST['ABcart_id']);
			} else {
	...
	public function testcron()
    {
        ...
        $this->template            = 'module/abandonedcarts/test_cron.php';
        $this->response->setOutput($this->render());
    }

	public function removeallrecords() {
		if (isset($_POST['remove']) && ($_POST['remove']==true) && isset($_POST['store']) ) {
				$run_query = $this->db->query("DELETE FROM `" . DB_PREFIX . "abandonedcarts` WHERE `store_id`='".$_POST['store']."'");
				if ($run_query) echo "Success!";
		}
	}
	...
	public function removeallemptyrecords() {
		if (isset($_POST['remove']) && ($_POST['remove']==true) && isset($_POST['store']) ) {
				$run_query = $this->db->query("DELETE FROM `" . DB_PREFIX . "abandonedcarts` WHERE `store_id`='".$_POST['store']."' AND `customer_info`=''");
				if ($run_query) echo "Success!";
		}
	}
	...
	private function listCoupons($action) {
		$this->load->language('module/abandonedcarts');
		...
		if($action == 'usedCoupons') {
			$coupon_total = $this->model_module_abandonedcarts->getTotalUsedCoupons();
			$coupons  = $this->model_module_abandonedcarts->getUsedCoupons($data);

		} else {
			$coupon_total  = $this->model_module_abandonedcarts->getTotalGivenCoupons();
			$coupons  =      $this->model_module_abandonedcarts->getGivenCoupons($data);
		}
		if(!empty($coupons)) {
		...
		$this->data['sort_name']       = 'index.php?route=module/abandonedcarts/'.$action.'&token=' . $this->session->data['token'] . '&sort=name' . $url;
		$this->data['sort_code']       = 'index.php?route=module/abandonedcarts/'.$action.'&token=' . $this->session->data['token'] . '&sort=code' . $url;
		$this->data['sort_discount']   = 'index.php?route=module/abandonedcarts/'.$action.'&token=' . $this->session->data['token'] . '&sort=discount' . $url;
		$this->data['sort_date_start'] = 'index.php?route=module/abandonedcarts/'.$action.'&token=' . $this->session->data['token'] . '&sort=date_start' . $url;
		$this->data['sort_date_end']   = 'index.php?route=module/abandonedcarts/'.$action.'&token=' . $this->session->data['token'] . '&sort=date_end' . $url;
		$this->data['sort_status']     = 'index.php?route=module/abandonedcarts/'.$action.'&token=' . $this->session->data['token'] . '&sort=status' . $url;
		$this->data['sort_email']      = 'index.php?route=module/abandonedcarts/'.$action.'&token=' . $this->session->data['token'] . '&sort=email' . $url;
		$this->data['sort_discount_type'] = 'index.php?route=module/abandonedcarts/'.$action.'&token=' . $this->session->data['token'] . '&sort=discount_type' . $url;
		$this->data['sort_date_added']   = 'index.php?route=module/abandonedcarts/'.$action.'&token=' . $this->session->data['token'] . '&sort=date_added' . $url;
		$url = '';
		if (isset($this->request->get['sort'])) {
				$url .= '&sort=' . $this->request->get['sort'];
		}
		...
		$pagination->text         = $this->language->get('text_pagination');
		$pagination->url          = 'index.php?route=module/abandonedcarts/'.$action.'&token=' . $this->session->data['token'] . $url . '&page={page}';
		$this->data['pagination'] = $pagination->render();
		$this->data['sort']       = $sort;
